...
def setup_gui():
    HEADERS = ['Topic', 'Min nb of pages', 'Max nb of pages', 'Potential readers (per page)']
    DEFAULT_DATA = [
    ...
    table = tksheet.Sheet(input_widgets, headers=HEADERS, total_columns=4, width=515, height=260, header_align='w', show_x_scrollbar=False)
    table.enable_bindings(['single_select','row_select','arrowkeys','right_click_popup_menu', 'edit_cell','copy','cut', 'delete', 'undo', "rc_insert_row","rc_delete_row"])
    table.grid()
    ...
    total_label.grid(row=1,column=0, sticky='e',  padx=5, pady=5,)
    total_input = tk.Entry(input_widgets)
    total_input.insert(0, DEFAULT_TOTAL_PAGES)
    total_input.grid(row=1,column=1)

    def solve():
        apply_model(table.get_sheet_data(return_copy=True)[:-1], # remove empty row
...
def show_bar_graph(topics, total, pages, readers):
    ...
    axes.set_title('Solución (Total páginas: %d, Número potencial de lectores: %d)' % (total, readers))
